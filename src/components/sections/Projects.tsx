...
const Projects: React.FC = () => {
  ...
  return (
    <section
      ...
      className="pt-24 pb-16 bg-gradient-to-br from-gray-50 to-white overflow-hidden"
    >
      <div className="container mx-auto px-4 lg:px-8">
        ...
        >
          <div className="relative inline-block">
            ...
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-40 h-1 bg-gradient-to-r from-[#9B4F96] to-[#c96bb3] rounded-full" />
          </div>
          <p
        ...
        <div
          ...
          className={`flex flex-wrap justify-center gap-4 mb-12 transition-all duration-1000 delay-200 transform ${
            isVisible['category-filter'] ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
          }`}
        >
          {categories.map((category) => (
            <button
              ...
              className={`px-8 py-3 rounded-full transition-all duration-300 font-semibold shadow-md hover:shadow-lg transform hover:scale-105 ${
        ...
        {loading ? (
          <div className="text-center py-16" role="status" aria-live="polite">
            <Loader
              className="inline-block w-12 h-12 text-[#9B4F96] animate-spin"
              aria-hidden="true"
            />
            <p
          ...
          <>
            ...
            <div
              id="projects-grid"
              data-animate
              className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 transition-all duration-1000 delay-400 transform ${
                isVisible['projects-grid'] ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
              }`}
            >
              {displayedProjects.map((project) => (
                ...
                >
                  ...
                  <div className="relative overflow-hidden h-64">
                    {project.images?.[0] ? (
                      <Image
                        ...
                        alt={`${project.title} - ${project.category} fit-out project by AQ Decor${project.location ? ` in ${project.location}` : ''}`}
                        fill
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-[#9B4F96]/20 to-[#c96bb3]/20 flex items-center justify-center">
                        <Eye className="text-[#9B4F96]" size={40} aria-hidden="true" />
                      </div>
                    )}
                    ...
                    <div className="absolute top-4 left-4 z-10">
                      <span
                        className="text-xs font-semibold text-white bg-gradient-to-r from-[#9B4F96] to-[#c96bb3] px-3 py-1 rounded-full shadow-lg"
                        style={{ fontFamily: '"Lucida Bright", Georgia, serif' }}
                      >
                        {project.category}
                      </span>
                    ...
                    {project.year && (
                      <div className="absolute top-4 right-4 z-10">
                        <span
                          className="text-xs font-semibold text-white bg-black/20 backdrop-blur-sm px-3 py-1 rounded-full"
                          style={{ fontFamily: '"Lucida Bright", Georgia, serif' }}
                        >
                          {project.year}
                        </span>
                  ...
                  <div className="p-6">
                    <h3
                      className="text-xl font-bold mb-3 text-[#9B4F96] group-hover:text-[#c96bb3] transition-colors duration-300"
                      style={{ fontFamily: '"Lucida Bright", Georgia, serif' }}
                    >
                      {project.title}
                    </h3>
                    <p
                      className="text-base text-gray-600 mb-4 line-clamp-2 leading-relaxed"
                      style={{ fontFamily: '"Lucida Bright", Georgia, serif' }}
                    >
                      {project.description}
                    </p>
                    {project.location && (
                      <div className="flex items-center text-gray-500 text-sm">
                        <MapPin size={14} className="mr-1 text-[#9B4F96]" aria-hidden="true" />
            ...
            {visibleProjects < filteredProjects.length && (
              <div
                ...
                className={`text-center mt-12 transition-all duration-1000 delay-600 transform ${
                  isVisible['load-more'] ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                }`}
              >
                <button
                  onClick={loadMore}
                  aria-label={`Load more projects, ${filteredProjects.length - visibleProjects} remaining`}
            ...
            {filteredProjects.length === 0 && !loading && (
              <div className="text-center mt-12">
                <div className="bg-gradient-to-r from-white to-purple-50 rounded-3xl p-12 shadow-lg border border-gray-100">
                  ...
                  <h3
                    className="text-3xl font-bold mb-3 text-[#9B4F96]"
                    style={{ fontFamily: '"Lucida Bright", Georgia, serif' }}
                  >
                    No projects found
                  </h3>
            ...
            <div
              id="stats-section"
              data-animate
              className={`mt-20 transition-all duration-1000 delay-800 transform ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8 bg-gradient-to-r from-white to-purple-50 rounded-3xl p-12 shadow-xl border border-gray-100">
                {[
                  ...
                  <div key={index} className="text-center group">
                    ...
                    <div
                      className="text-4xl font-bold text-[#9B4F96] mb-2"
                      style={{ fontFamily: '"Lucida Bright", Georgia, serif' }}
                    >
                      {stat.number}
                    </div>
